...
import { fetchActivityLogs, setFilters, setPage } from '../../../store/slices/activityLogSlice';
import { Calendar, Filter, Search, ChevronLeft, ChevronRight } from 'lucide-react';
import './AdminActivityLog.css';

const AdminActivityLog = () => {
  const dispatch = useDispatch();
  const { logs, loading, error, filters, pagination } = useSelector((state) => state.activityLog);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    dispatch(fetchActivityLogs({ ...filters, page: pagination.currentPage, search: searchTerm }));
    ...
    <div className="admin-activity-log">
      <div className="activity-log-header">
        <p className="activity-log-description">
          A detailed record of all admin activity for clear oversight and compliance.
        </p>
      </div>
