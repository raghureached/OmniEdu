...
exports.getScormModule = async (req, res) => {
  try {
    const module = await ScormModule.findById(req.params.id)
      .populate('team', 'name')
      .populate('subteam', 'name')
      .populate('createdBy', 'name email');
...
exports.updateScormModule = async (req, res) => {
  try {
    ...
    const updatedModule = await ScormModule.findByIdAndUpdate(
      req.params.id,
      updateData,
...
exports.deleteScormModule = async (req, res) => {
  try {
    const module = await ScormModule.findByIdAndDelete(req.params.id);
...
exports.launch = async (req, res) => {
  try {
    ...
    const module = await ScormModule.findById(moduleId);
    if (!module) {
