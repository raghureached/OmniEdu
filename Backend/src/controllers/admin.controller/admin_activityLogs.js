...
const getActivityLogs = async (req, res) => {
  try {
    ...
    // Exclude GlobalAdmin role logs by default
    ...
    // Filter by user role if provided (but still exclude GlobalAdmin)
...
const getActivityLogStats = async (req, res) => {
  try {
    // Get activity statistics (excluding GlobalAdmin)
    const stats = await ActivityLog.aggregate([
      {
        $match: {
          userRole: { $ne: "GlobalAdmin" }
        }
      },
      {
        $group: {
...
const testDatabase = async (req, res) => {
  try {
    const totalLogs = await ActivityLog.countDocuments({ userRole: { $ne: "GlobalAdmin" } });
    const sampleLogs = await ActivityLog.find({ userRole: { $ne: "GlobalAdmin" } }).limit(5).populate('user', 'name email');
