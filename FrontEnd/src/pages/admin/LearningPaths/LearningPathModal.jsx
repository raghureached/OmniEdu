...
import { ChevronLeft, ChevronRight, Plus, Trash2, GripVertical, Package, X } from 'lucide-react';
...
const LearningPathModal = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  const assessments = assessmentsRaw.length ? assessmentsRaw : [
    { uuid: 'a-201', type: 'assessment', title: 'JS Quiz - Level 1' },
    { uuid: 'a-202', type: 'assessment', title: 'React Quiz - Hooks' },
  ];
  const surveys = surveysRaw.length ? surveysRaw : [
    { uuid: 's-301', type: 'survey', title: 'Module Feedback Survey' },
    { uuid: 's-302', type: 'survey', title: 'Onboarding Experience Survey' },
  ];
  ...
  const itemsByType = {
    module: modules.map(m => ({ id: m.uuid || m.id, title: m.title })),
    assessment: assessments.map(a => ({ id: a.uuid || a.id, title: a.title })),
    survey: surveys.map(s => ({ id: s.uuid || s.id, title: s.title })),
  };
  ...
  const handleSubmit = (e) => {
    ...
    const lessons = pathItems.map((it, index) => ({ id: it.id, type: it.type, title: it.title, order: index }));
    const payload = {
      ...form,
      tags,
      lessons,
              ...
              <div className="module-overlay__step">
                          ...
                          {itemsByType[pickerType]
                            ...
                            .map(x => (
                              <button
                                key={x.id}
                                type="button"
                                className="lp-picker-row"
                                onClick={() => { setPathItems(prev => [...prev, { type: pickerType, id: x.id, title: x.title }]); setPickerOpen(false); }}
                              >
                ...
                <div className="module-overlay__form-group">
                  {(() => {
                    ...
                    // Prefer summing item durations if present, else fall back to overall form.duration
                    const perItemSum = pathItems.reduce((acc, it) => {
                      // try to find duration from sources if available in future
                      return acc + (Number(it.duration) || 0);
                    }, 0);
                    const totalDuration = perItemSum > 0 ? perItemSum : Number(form.duration || 0);
                    return (
              ...
              <div className="module-overlay__step">
                ...
                <div className="lp-grid-4">
                  <div className="module-overlay__form-group">
                    ...
                    <input className="addOrg-form-input" type="number" min={0} name="duration" value={form.duration} onChange={handleChange} placeholder="Auto computed" disabled />
                  </div>
                  <div className="module-overlay__form-group">
            ...
            <div className="module-overlay__step-navigation">
              ...
              {currentStep < totalSteps ? (
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button type="submit" className="btn-primary" onClick={(e)=>handleSubmit(e)}>{initialData ? 'Save Changes' : 'Create'}</button>
                </div>
              )}
            </div>
          </div>
