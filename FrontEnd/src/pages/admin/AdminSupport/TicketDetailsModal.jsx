...
import { fetchAdminTicketDetails, addAdminTicketComment, clearCurrentTicket } from '../../../store/slices/adminTicketsSlice';

export default function TicketDetailsModal({ ticketId, onClose }) {
  const dispatch = useDispatch();
  const { currentTicket: ticket, detailsLoading, commentSending, error } = useSelector(state => state.adminTickets);
  const [comment, setComment] = useState('');
  const [commentAttachments, setCommentAttachments] = useState([]);

  useEffect(() => {
    if (ticketId) {
      dispatch(fetchAdminTicketDetails(ticketId));
    }
  ...
  const handleSendComment = async () => {
    ...
    try {
      await dispatch(addAdminTicketComment({ 
        ticketId, 
        message: comment, 
