...
const ScormModuleModal = ({ showModal, setShowModal, teams, onSuccess }) => {
  const totalSteps = 3;
  ...
  const handleCreate = async () => {
    try {
      ...
      await api.post("/api/scorm/upload", fd);
      notifySuccess("SCORM module created successfully");
      setShowModal(false);
      onSuccess?.();
    } catch (err) {
      notifyError("Failed to create SCORM module");
    } finally {
      setUploading(false);
    }
  };
        ...
        <div className="module-overlay__footer">
          <div className="module-overlay__step-navigation">
            ...
            {currentStep < totalSteps ? (
              ...
              >
                {uploading ? "Creating..." : "Create SCORM Module"}
              </button>
            )}
          </div>
        </div>
