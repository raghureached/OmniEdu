...
const OrganizationFormModal = ({
  ...
  const openDocumentPreview = (fileOrUrl) => {
    if (fileOrUrl instanceof Blob) {
      ...
      // Assume it is a remote/cloud URL
      setDocPreviewUrl(fileOrUrl);
    } else {
      console.error("Invalid document type for preview", fileOrUrl);
      ...
      {showLogoPreview && selectedLogo && (
        <div className="addOrg-preview-overlay" onClick={() => setShowLogoPreview(false)}>
          <div className="addOrg-preview-content" onClick={e => e.stopPropagation()}>
            ...
            <img
              src={typeof selectedLogo === "string" ? selectedLogo : URL.createObjectURL(selectedLogo)}
              alt="Logo Preview"
